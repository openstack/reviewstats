...
for project in ${projects} ; do
	...
	(date -u && echo && ./openreviews.py -p ${project} ${EXTRA_ARGS}) > results/${project_base}-openreviews.txt
	./openreviews.py -p ${project} --html ${EXTRA_ARGS} > results/${project_base}-openreviews.html
	(date -u && echo && ./openapproved.py -p ${project} ${EXTRA_ARGS}) > results/${project_base}-openapproved.txt
done

if [ "${all}" = "1" ] ; then
	(date -u && echo && ./openreviews.py -a ${EXTRA_ARGS}) > results/all-openreviews.txt.tmp
	...
	(date -u && echo && ./openapproved.py -a ${EXTRA_ARGS}) > results/all-openapproved.txt
fi
